var NativeLibpqWrapper = require('bindings')('addon.node').PQNative;
var util = require("util");
var assert = require('assert');

var EventEmitter = require('events').EventEmitter;
// util.inherits(PQ, EventEmitter);
...
// class PQ extends PQNative {
//     constructor(...args) {
//         super(...args);
//         this.connected = false;
//     }
//
//     connect (paramString, cb) {
//       this.connected = true;
...
//       }
//       super.$connect(paramString, cb);
//     }
// }
class PQ extends EventEmitter {
    constructor () {
        super();
        this.native = new NativeLibpqWrapper(this.emit.bind(this));
        this.connected = false;
    }


    connectSync (paramString) {
      this.connected = true;
      if(!paramString) {
        paramString = '';
      }
      var connected = this.native.$connectSync(paramString);
      if(!connected) {
    ...
    connect (paramString, cb) {
      ...
      this.native.$connect(paramString, cb);
    }

    errorMessage () {
      return this.native.$getLastErrorMessage();
    }

    //returns an int for the fd of the socket
    socket () {
      return this.native.$socket();
    };

    // return server version number e.g. 90300
    serverVersion () {
      return this.native.$serverVersion();
    };
    ...
    //returns false if there was an error
    //consume additional error details via PQ#errorMessage & friends
    execParams (commandText, parameters) {
      if(!commandText) {
        commandText = '';
      }
      if(!parameters) {
      ...
      this.native.$execParams(commandText, parameters);
    }
    ...
    //returns false if there was an error
    //consume additional error details via PQ#errorMessage & friends
    prepare (statementName, commandText, nParams) {
      ...
      this.native.$prepare(statementName, commandText, nParams);
    }
    ...
    execPrepared (statementName, parameters) {
      ...
      this.native.$execPrepared(statementName, parameters);
    }
    ...
    sendQuery (commandText) {
      ...
      return this.native.$sendQuery(commandText);
    }

    //send a command to begin executing a query with parameters in async mode
    //returns true if sent, or false if there was a send failure
    sendQueryParams (commandText, parameters) {
      if(!commandText) {
        commandText = '';
      }
      if(!parameters) {
      ...
      return this.native.$sendQueryParams(commandText, parameters);
    }

    //send a command to prepare a named query in async mode
    //returns true if sent, or false if there was a send failure
    sendPrepare (statementName, commandText, nParams) {
      assert.equal(arguments.length, 3, 'Must supply 3 arguments');
      if(!statementName) {
        statementName = '';
      }
      ...
      nParams = Number(nParams) || 0;
      return this.native.$sendPrepare(statementName, commandText, nParams);
    }

    //send a command to execute a named query in async mode
    //returns true if sent, or false if there was a send failure
    sendQueryPrepared (statementName, parameters) {
      if(!statementName) {
        statementName = '';
      }
      if(!parameters) {
      ...
      return this.native.$sendQueryPrepared(statementName, parameters);
    }
    ...
    getResult () {
      return this.native.$getResult();
    }

    //returns a text of the enum associated with the result
    //usually just PGRES_COMMAND_OK or PGRES_FATAL_ERROR
    resultStatus () {
      return this.native.$resultStatus();
    }

    resultErrorMessage () {
      return this.native.$resultErrorMessage();
    }

    resultErrorFields () {
      return this.native.$resultErrorFields();
    }
    ...
    getvalue (row, col) {
      return this.native.$getvalue(row, col);
    }

    //returns true/false if the value is null
    getisnull (row, col) {
      return this.native.$getisnull(row, col);
    }

    //returns the status of the command
    cmdStatus = function() {
      return this.native.$cmdStatus();
    }

    //returns the tuples in the command
    cmdTuples = function() {
      return this.native.$cmdTuples();
    }
    ...
    consumeInput () {
      return this.native.$consumeInput();
    };
    ...
    isBusy () {
      return this.native.$isBusy();
    }

    //toggles the socket blocking on outgoing writes
    setNonBlocking (truthy) {
      return this.native.$setNonBlocking(truthy ? 1 : 0);
    }
    ...
    isNonBlocking () {
      return this.native.$isNonBlocking();
    }
    ...
    escapeLiteral (input) {
      ...
      return this.native.$escapeLiteral(input);
    }

    escapeIdentifier (input) {
      if(!input) return input;
      return this.native.$escapeIdentifier(input);
    }
    ...
    putCopyData (buffer) {
      ...
      return this.native.$putCopyData(buffer);
    }
    ...
    //returns -1 if there was an error
    putCopyEnd (errorMessage) {
      if(errorMessage) {
        return this.native.$putCopyEnd(errorMessage);
      }
      return this.native.$putCopyEnd();
    }
    ...
    getCopyData (async) {
      return this.native.$getCopyData(!!async);
    }
